...
export default class extends Controller {
  ...
  makeMarked(marked, cx= 55, cy = 55, r = 55){
    ...
    clockMarked.setAttribute("stroke", "blue")
    clockMarked.setAttribute("stroke-width", "50")
    clockMarked.setAttribute("stroke-dasharray", `calc(${this.markedValue * 300} * ${this.sectionsValue} / 100) 157.1`)
  ...
  makeSections(sections, cx, cy, r){
    let clockLines = this.createSvg("svg")
        for (let i=1; i<=sections; i++){
            ...
            clockLine.setAttribute("stroke-width", "4")
            clockLine.setAttribute("d", `M${cx} ${cy} ${cx + r * Math.sin((2 * Math.PI * i) / sections)} ${cy + r * (-1 * Math.cos((2 * Math.PI * i) / sections))}`)
